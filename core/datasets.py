...
class Sintel(data.Dataset):
    def __init__(self, aug_params=None, split: Literal['training', 'validate'] = 'training', root='datasets/Sintel', dstype='clean'):
        ...
        for scene in os.listdir(image_root):
            ...
            image_files = sorted(glob(osp.join(image_root, scene, '*.png')))
            n_pairs = len(image_files) - 1
            for i in range(n_pairs):
                image_list += [[image_files[i], image_files[i+1]]]
            flow_list += sorted(glob(osp.join(flow_root, scene, '*.flo')))
            invalid_list += sorted(glob(osp.join(invalid_root,
                                                 scene, '*.png')))
    ...
    def __getitem__(self, index) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        invalid = frame_utils.read_gen(self.invalid_list[index])

        img1 = frame_utils.read_gen(self.image_list[index][0])
        img2 = frame_utils.read_gen(self.image_list[index][1])

        occ = np.array(occ).astype(bool)
        invalid = np.array(invalid).astype(bool)
        flow = np.array(flow).astype(np.float32)
        img1 = np.array(img1).astype(np.uint8)
        img2 = np.array(img2).astype(np.uint8)

        valid = np.logical_not(np.logical_or(occ, invalid))
