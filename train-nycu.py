...
from utils.utils import create_flow_grid, fwdbwd_occ_mask, photometric_error, photometric_error_masked
...
def sequence_loss(flow_preds_fwd: List[torch.Tensor],
                  flow_preds_bwd: List[torch.Tensor],
    ...
    for i in range(n_predictions):
        ...
        flow_fwd = flow_preds_fwd[i]
        flow_bwd = flow_preds_bwd[i]

        vgrid_fwd = create_flow_grid(flow_fwd)
        vgrid_bwd = create_flow_grid(flow_bwd)
        mask_fwd, mask_bwd, warpdiff_fwd, warpdiff_bwd = fwdbwd_occ_mask(flow_fwd, flow_bwd,
                                                                         vgrid_fwd, vgrid_bwd,
                                                                         use_border_mask=True,
                                                                         return_warpdiff=True)
        mask_fwd = torch.clamp(mask_fwd, min=1e-3)    # prevent collapse
        mask_bwd = torch.clamp(mask_bwd, min=1e-3)    # prevent collapse

        mag_fwd = torch.sum(flow_fwd**2, dim=1).sqrt()
        mag_bwd = torch.sum(flow_bwd**2, dim=1).sqrt()
        valid = (mag_fwd < max_flow) & (mag_bwd < max_flow)

        # Forward loss
        mask_fwd = mask_fwd * valid[:, None]
        l1_err, ssim_err = photometric_error_masked(image1, image2,
                                                    vgrid_fwd, mask_fwd)
        i_loss = (1 - SSIM_WEIGHT) * l1_err + SSIM_WEIGHT * ssim_err
        flow_loss += i_weight * i_loss

        # Backward loss
        mask_bwd = mask_bwd * valid[:, None]
        l1_err, ssim_err = photometric_error_masked(image2, image1,
                                                    vgrid_bwd, mask_bwd)
        i_loss = (1 - SSIM_WEIGHT) * l1_err + SSIM_WEIGHT * ssim_err
        flow_loss += i_weight * i_loss

        # Fwd-Bwd Consistency
        flow_loss += 0.2 * (torch.mean(torch.abs(warpdiff_fwd) * mask_fwd) / mask_fwd.mean()
                            + torch.mean(torch.abs(warpdiff_bwd) * mask_bwd) / mask_bwd.mean())
...
def train(args):
    train_loader = datasets.fetch_nycu(args)
    ...
    for epoch in range(epoch_start, args.num_epochs):
        ...
        for batch_idx, data_blob in enumerate(train_loader):
            ...
            flow_predictions_fwd, _ = model(image1, image2,
                                            iters=args.iters)

            flow_predictions_bwd, _ = model(image2, image1,
                                            iters=args.iters)

            loss, metrics = sequence_loss(flow_predictions_fwd, flow_predictions_bwd,
                                          image1, image2, args)
